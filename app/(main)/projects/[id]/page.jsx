...
export default function Project() {
  ...
  const [_, setSelectedImage] = useState("");
  ...
  const previousProjectExists = projects.some(
    (project) => project.id === buildProjectId(-1)
  );
  const nextProjectExists = projects.some(
    (project) => project.id === buildProjectId(1)
  );
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:title"
          content={`Archer Arquitectura | ${selectedProject.title}`}
          key="title"
        />
        <meta
          name="description"
          content={`Descubre ${selectedProject.title} y todos los proyectos en los que ha participado a lo largo de su trayectoria Archer Arquitectura, estudio de arquitectura ubicado en Madrid.`}
        />
          ...
          <section className="w-full md:w-1/2 flex flex-col justify-start">
            ...
            <nav className="flex flex-row justify-between pb-sp6">
              {previousProjectExists ? (
                <Link legacyBehavior href={`/projects/${buildProjectId(-1)}`}>
                  <span className="flex flex-row items-center text-paragraph transition-all font-roboto font-regular hover:text-primary-link-hover cursor-pointer gap-sp2">
                    <FaChevronLeft />
                    <a>Anterior</a>
                  </span>
                </Link>
              ) : (
                <span className="flex flex-row items-center text-paragraph transition-all font-roboto font-regular text-gray-400 cursor-not-allowed gap-sp2">
                  <FaChevronLeft />
                  Anterior
                </span>
              )}
            ...
            <section className="flex flex-row gap-sp4">
              <h1
                ...
                role="heading"
                aria-level={1}
                className="w-fit font-futura-light text-big-paragraph mb-sp4 bg-black text-white rounded-md px-sp4 py-sp2"
              >
                {selectedProject.title}
              </h1>
              <p
                ref={yearRef}
                className="w-fit font-futura-light text-big-paragraph mb-sp4 border border-black bg-transparent text-black rounded-md px-sp4 py-sp2"
              >
            ...
            <section className="font-roboto text-primary-font text-paragraph flex flex-row flex-wrap gap-sp1 mb-sp1">
              ...
              <a
                className="font-medium primary-link underline"
            ...
            {/* Description */}
            <section className="pb-sp16">
              {selectedProject.description &&
                selectedProject.description.map((desc, index) => {
                  return (
                    <p
                      className="mb-sp3 font-roboto text-paragraph font-light text-justify"
                      key={index}
                    >
                      {desc}
                    </p>
